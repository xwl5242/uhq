...
class TBApi:
    ...
    @staticmethod
    def get_goods_list(material_id, page_no=1, page_size=15):
        """
        获取指定物料类型的精选商品信息
        ...
        try:
            ...
            for g in g_list:
                ...
                coupon_amount = g.get('coupon_amount')
                good['coupon_amount'] = coupon_amount if coupon_amount else '??'  # 优惠券额度
    ...
    @staticmethod
    def search_item(keyword, page_no=1, page_size=15):
        """
        搜索物料信息
        ...
        try:
            ...
            resp = req.getResponse()
            # 返回结果
            g_list = resp.get('tbk_dg_material_optional_response').get('result_list').get('map_data')
            # 由于结果中的一口价为None，所以需要再次调用获取物料详情接口
            item_ids = ','.join([str(g.get('item_id')) for g in g_list])
            # 获取物料详情
            items = TBApi.get_item_info(item_ids)
            ...
            for g in g_list:
                good = dict()
                item_id = g.get('item_id')  # 商品id
                good['coupon_id'] = g.get('coupon_id')
                good['item_id'] = g.get('item_id')
                ...
                good['coupon_remain_count'] = g.get('coupon_remain_count')
