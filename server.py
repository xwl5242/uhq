...
def index():
    ...
    return AppServer.render_page('index.html', ROOT, today_rec=top[:5], banners=top[5:], txs=txs, cur_q=ROOT)
...
def menu_html(q_type):
    ...
    return AppServer.render_page('index.html', q_type, today_rec=top[:5], banners=top[5:], txs=txs, cur_q=q_type)
...
def menu_nav_html(q_type_item):
    ...
    return AppServer.render_page('index.html', cur_q_i, today_rec=top[:5],
                                 banners=top[5:], txs=txs, cur_q=cur_q, cur_q_i=cur_q_i)
